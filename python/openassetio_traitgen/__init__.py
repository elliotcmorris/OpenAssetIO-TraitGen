...
def generate(
    ...
    # Don't allow empty string to fall through as getattr will throw.
    if not generator:
        logger.error("No generator specified")
        return

    # Retrieve the generator by looking up an attribute with the
    # requested name and generate.
    generator_obj = getattr(generators, generator, None)
    if generator_obj is None:
        logger.error("Cannot find generator %s", generator)
        return
    ...
    logger.info("Generating with %s...", generator)
    child_logger = logger.getChild(generator)
